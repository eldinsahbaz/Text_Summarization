...
def clean_data(cutoff, path, target):
    def filter_symbols(text):
        text = re.sub(r'https?:\/\/.*[\r\n]*', '', text, flags=re.MULTILINE)
        text = re.compile(r'(<!--.*?-->|<[^>]*>)').sub('', text)
        text = re.sub(r'&amp;', '', text)
        text = re.sub('\.\.\.', '.', text)
        text = [nltk.word_tokenize(re.sub(r'[^a-zA-Z ]+', '', tokens)) for tokens in text.split('.')]

        return text

    data = pd.read_csv(path)[['Summary', 'Text']].values.tolist()
    stop_words = set(stopwords.words('english'))
    lemmatizer = WordNetLemmatizer()
    cleaned = list()

    for index, (summary, review) in enumerate(data):
        print(index, len(data))
        temp = list()

        try: temp.append(contractions.fix(summary.lower()))
        except: continue

        try: temp.append(contractions.fix(review.lower()))
        except: continue

        edited_summary = [[str(TextBlob(word2).correct()) for word2 in word1] for word1 in filter_symbols(temp[0])]
        ...
        if (not(index%1000) and index > 0):
            ...
            _ = create_embeddings(cleaned, cutoff, "Embedding_Map.txt", "Embedded_Summary.txt", "Embedded_Review.txt")
...
with tf.device('/device:GPU:0'):
    if (__name__ == '__main__') and (len(sys.argv) > 1):
        if 'train' == sys.argv[1]:
            cutoff = 15
            DNS, embedded_summaries, embedded_reviewsmax_summary_length, max_review_length, min_length, unk_review_limit, unk_summary_limit, unk_summary_limit = create_embeddings(clean_data(cutoff, 'Reviews.csv', 'cleaned_2.txt'), cutoff, "Embedding_Map.txt", "Embedded_Summary.txt", "Embedded_Review.txt")
