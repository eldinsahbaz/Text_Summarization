...
with tf.device('/device:GPU:0'):
    if (__name__ == '__main__') and (len(sys.argv) > 1):
        if 'train' == sys.argv[1]:
            cutoff = 15
            vals_to_unpack = create_embeddings(clean_data('Reviews.csv', 'cleaned_2.txt'), cutoff, "Embedding_Map.txt", "Embedded_Summary.txt", "Embedded_Review.txt")
            #DNS, embedded_summaries, embedded_reviewsmax_summary_length, max_review_length, min_length, unk_review_limit, unk_summary_limit, unk_summary_limit
